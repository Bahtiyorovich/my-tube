...
const VideoCard = ({video}) => {
  ...
  return (
    <Card sx={{width: '350px', boxShadow: 'none', borderRadius:0}}>
        <CardMedia image={video?.snippet?.thumbnails?.high?.url} alt={video?.snippet?.title} sx={{width: '300px', height: '158px', borderRadius: '15px', marginLeft: '10px'}}/>
        <CardContent sx={{background: colors.cardColor, height: '180px', position: 'relative'}}>
            <Stack direction={'row'}>
              <Avatar src={video?.snippet?.thumbnails?.high?.url} sx={{marginRight: '20px', marginBottom: '15px'}}/>
            ...
            <Typography variant='subtitle2' my={'5px'} sx={{opacity: '0.7', marginLeft: 7, fontWeight: 400}}>
